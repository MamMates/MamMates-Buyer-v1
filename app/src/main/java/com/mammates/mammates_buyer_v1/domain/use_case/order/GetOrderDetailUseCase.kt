...
import com.mammates.mammates_buyer_v1.util.toOrderDetail
...
class GetOrderDetailUseCase @Inject constructor(
    ...
    operator fun invoke(
        token: String,
        id: Int
    ): Flow<Resource<OrderDetail>> = flow {
        try {
            emit(Resource.Loading())
            val orderDetail = orderRepository.getOrderDetail(token, id).data?.toOrderDetail()
            orderDetail?.let {
